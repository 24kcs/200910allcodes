 ...
 * 
 * 
 * 声明式路由:<router-link> (其实直接使用a标签也可以) 配合<router-view> 不需要使用js代码,仅仅是通过html代码就可以实现路由跳转
 * 编程式路由:通过js代码的方式来实现路由跳转(依靠于router路由器对象)
 * 
 * 声明式路由跳转和编程式路由跳转中的push的方式 实现路由跳转的效果一样
 * 
 * 编程式路由有两种方式: push方式和replace方式
 * push方式有历史记录
 * replace方式属于替换操作
 * 声明式路由<router-link> 默认的跳转效果和push是一样的
 * 但是<router-link replace> 这么书写,此时跳转效果和replace是一样的
 * 
 * 
 * 路由在注册的时候,可以使用props函数的方式来设定传递的参数,在对应的组件中使用props的方式接收,并使用
 * 
 * 
 * <router-view> 中可以向某个跳转的组件中传递数据,哪个组件需要这个数据,就使用props来进行接收即可
 * 
 * 
 * 
 * 
